<?php
    ...
    */
	...
	// step 2
	$url = "https://localhost/sommelier/api/oauth2/token"; // url to send data to for authentication
	$headers = array( 
...
function invalidate_bearer_token($bearer_token){
    // step 1 - encode the keys & make the consumer token
    $encoded_consumer_key = urlencode(CONSUMER_KEY);
    $encoded_consumer_secret = urlencode(CONSUMER_SECRET);
    ...
    $url = "https://localhost/sommelier/api/oauth2/invalidate_token"; // url to send data to for authentication
    $headers = array( 
