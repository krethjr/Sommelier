<?php

/**
 * This is the model class for table "{{cellar_location}}".
 *
 * The followings are the available columns in table '{{cellar_location}}':
 * @property integer $cellar_loc_id
 * @property integer $cellar_id
 * @property string $location
 * @property string $location_desc
 *
 * The followings are the available model relations:
 * @property Cellars $cellar
 * @property CellarWines[] $cellarWines
 */
class CellarLocation extends CActiveRecord
{
	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return '{{cellar_location}}';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('location', 'required'),
			array('cellar_loc_id, cellar_id', 'numerical', 'integerOnly'=>true),
			array('location', 'length', 'max'=>10),
			array('location_desc', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('cellar_loc_id, cellar_id, location, location_desc', 'safe', 'on'=>'search'),
                        // added custom validator to make sure we don't delete the defualt location for the cellar
			array('cellar_loc_id', 'validateDefLoc', 'on'=>'delete'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
			'cellar' => array(self::BELONGS_TO, 'Cellars', 'cellar_id'),
			'cellarWines' => array(self::HAS_MANY, 'CellarWines', 'cellar_loc_id'),
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'cellar_loc_id' => 'Cellar Loc',
			'cellar_id' => 'Cellar',
			'location' => 'Location',
			'location_desc' => 'Location Desc',
		);
	}

        /**
         * This custom validator is used to prevent the deletion of a cellar location
         * where that location is the curren default for a cellar
         */
        public function validateDefLoc()
        {
            $currentCellar=Cellars::model()->findByPk($this->cellar_id);

            if ($this->cellar_loc_id == $currentCellar->default_cellar_loc_id)
                return false;
            else
                return true;
        }
        
	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 *
	 * Typical usecase:
	 * - Initialize the model fields with values from filter form.
	 * - Execute this method to get CActiveDataProvider instance which will filter
	 * models according to data in model fields.
	 * - Pass data provider to CGridView, CListView or any similar widget.
	 *
	 * @return CActiveDataProvider the data provider that can return the models
	 * based on the search/filter conditions.
	 */
	public function search()
	{
		// @todo Please modify the following code to remove attributes that should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('cellar_loc_id',$this->cellar_loc_id);
		$criteria->compare('cellar_id',$this->cellar_id);
		$criteria->compare('location',$this->location,true);
		$criteria->compare('location_desc',$this->location_desc,true);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}

	/**
	 * Returns the static model of the specified AR class.
	 * Please note that you should have this exact method in all your CActiveRecord descendants!
	 * @param string $className active record class name.
	 * @return CellarLocation the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}
}
