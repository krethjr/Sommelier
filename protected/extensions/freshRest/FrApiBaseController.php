...
class FrApiBaseController extends CController
{
    ...
    /**
     * Convert error messages into readable string
     * submitted by https://bitbucket.org/zqi2
     */
    protected function errs($errors)
    {
        // convert variable to string
        $outval = print_r($errors, true);
        // replace one or more new line into a space character
        $outval = trim(preg_replace('/\s+/', ' ', $outval));
        return $outval;
    }
    ...
    public function actionUpdate($id)
    {
        ...
        {
            $this->renderError(400, $this->errs($apiModel->getErrors()));
        }
    }
    ...
    public function missingAction($actionID)
    {
        ...
        else
        {
            ...
            elseif ($action == 'create' && $requestType == 'POST')
            {
                $this->run('create');
            }
    ...
    public function handleError($event)
    {
        ...
        if (isset($event->exception))
        {
            ...
            elseif ($event->exception->getCode())
            {
    ...
    protected function renderOutput($data, $code = 200)
    {
        ...
        elseif ($this->module->format == 'jsonp')
        {
            throw new Exception("not implemented yet", 500);
        }
        elseif ($this->module->format == 'xml')
        {
            throw new Exception("not implemented yet", 500);
        }
        Yii::app()->end();
